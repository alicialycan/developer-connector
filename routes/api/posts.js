...
router.get('/', auth, async (req, res) => {
  try {
    // NOTE: sort by oldest posts first
...
router.delete('/:id', auth, async (req, res) => {
  try {
    const post = await Post.findById(req.params.id);
    // NOTE: Check if post exists
    if (!post) {
      return res.status(404).json({ msg: 'Post not found' });
    }
    // NOTE: Check if user exists
    if (post.user.toString() !== req.user.id) {
      return res.status(401).json({ msg: 'User is not authorizaed' });
    }
...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    ...
    // NOTE: Check if post has already been liked
    if (post.likes.filter(like => like.user.toString() === req.user.id).length === 0) {
...
router.post('/comment/:id', 
  [
    ...
    // NOTE: Add comment to the beginning of comments
